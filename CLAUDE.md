...
Phase history lives in [CHANGELOG.md](./CHANGELOG.md). Quick index:

| Phase | Summary |
|-------|---------|
| 1–3 | Basic HTML marking → DOM-aware snippet matching → Click CLI + pytest + `src/` layout |
| 4 | Advanced PDF extraction (PyMuPDF + pdfplumber, word segmentation, HTML matching) |
| 5 | Production PDF support: InDesign metadata filtering, dehyphenation, validation |
| 6 | Magazine support: page offset, footnote filtering, partial word completion, context correction |
| 7 | CLI simplification: HTML required by default; `--raw-pdf`, `--fuzzy-match` flags |
| 8 | Sequential position tracking for duplicate snippets + `--start-page`/`--end-page` filtering |
| 9 | **The Offset Hack** — `end_of_page` + `offset+1` for correct EPUB page-begins semantics |
| 10 | Context matching with `context_before`/`context_after` + Jaccard scoring for disambiguation |
| 11 | Visual marker editor (`tools/page-marker-editor.html`) — single-file, offline, drag-and-drop |
| 12 | PDF splitting (`rx-pagemarker split` + `tools/pdf-splitter.html`) — independent from marking |

The full per-phase details (commit refs, flag inventories, success rates) are in CHANGELOG.md.

See `Technical Decisions` below for the *why* behind the Offset Hack (Phase 9) and the two-column warning — those are the non-obvious bits that get re-read every session.
